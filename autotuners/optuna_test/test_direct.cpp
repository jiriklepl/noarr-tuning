...
struct tundis {
	NOARR_TUNE_BEGIN(optuna_formatter(
		std::cout,
		std::make_shared<noarr::tuning::direct_compile_command_builder>("g++", "-O2 -g -std=c++20 -o tmp.bin -I../../../include " __FILE__),
...
void run_matmul(A ta, B tb, C tc, num_t *pa, num_t *pb, num_t *pc) {
	...
	auto trav = noarr::traverser(a, b, c).order(T.block_i.get() ^ T.block_j.get() ^ T.block_k.get());
	...
	[=]<std::size_t ...Idxs>(auto order, std::index_sequence<Idxs...>){
		trav.template for_dims<(char)order.get().template get<Idxs>()...>(matmul(a, b, c));
	}(T.block_order, std::make_index_sequence<std::tuple_size_v<std::remove_cvref_t<decltype(T.block_order.get())>>>());
}
...
int main(int argc, char **argv) {
	...
	auto ta = noarr::scalar<num_t>() ^ T.a_order.get() ^ noarr::set_length<'i', 'k'>(size, size);
	auto tb = noarr::scalar<num_t>() ^ T.b_order.get() ^ noarr::set_length<'k', 'j'>(size, size);
	auto tc = noarr::scalar<num_t>() ^ T.c_order.get() ^ noarr::set_length<'i', 'j'>(size, size);
