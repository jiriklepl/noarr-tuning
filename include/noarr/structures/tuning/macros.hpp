...
#define NOARR_TUNE_PAR(parameter_name, ...) \
	struct NOARR_PARAMETER(parameter_name) { static constexpr const char *name= #parameter_name; static constexpr auto value_ = noarr::flexible_contain(NOARR_PARAMETER(VALUE_ ## parameter_name)); static constexpr auto value = std::integer_constant<decltype(value), value>; }; \
